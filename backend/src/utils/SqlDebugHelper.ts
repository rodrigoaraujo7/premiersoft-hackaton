...
export class SqlDebugHelper {

  /**
   * Analisa SQL problemático e identifica issues
   */
  static analyzeSql(sql: string): void {
   ...
   */
  static testInsert(tableName: string, data: Record<string, any>): void {
    try {
      console.log('=== TESTE INSERT ===');
      console.log('Dados originais:', data);

      const sanitized = SqlValidationService.validateAndSanitizeData(data);
      console.log('Dados sanitizados:', sanitized);

      const sql = SqlValidationService.buildInsertStatement(tableName, sanitized);
